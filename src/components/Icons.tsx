...
// Base function to handle size prop consistently
const getSizeProps = (className: string = 'w-6 h-6', size?: number) => {
  if (size) {
    return {
      className: className,
      width: size,
...
// Additional icons to replace lucide-react icons
export const MenuIcon: React.FC<IconProps> = ({ className = 'w-6 h-6', size }) => {
  const props = getSizeProps(className, size);
  return (
...
export const LoaderIcon: React.FC<IconProps> = ({ className = 'w-6 h-6', size }) => {
  const props = getSizeProps(className, size);
  return (
    <svg xmlns="http://www.w3.org/2000/svg" {...props} fill="none" viewBox="0 0 24 24" stroke="currentColor">
